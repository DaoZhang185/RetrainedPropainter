...
def read_mask(mpath, length, size, flow_mask_dilates=8, mask_dilates=5):
    ...
    for mask_img in masks_img:
        if size is not None:
            # [修改 1] 使用 BILINEAR 插值，防止 Resize 产生锯齿
            mask_img = mask_img.resize(size, Image.BILINEAR)

        mask_img = np.array(mask_img.convert('L'))

        # [修改 2] 使用 OpenCV 进行膨胀 (形状更圆润)
        # 1. 基础二值化
        _, mask_binary = cv2.threshold(mask_img, 127, 255, cv2.THRESH_BINARY)

        # 2. 定义圆形卷积核 (Kernel)
        # flow_mask 稍微大一点
        kernel_flow = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,
                                                (2 * flow_mask_dilates + 1, 2 * flow_mask_dilates + 1))
        # inpaint_mask 正常大小
        kernel_mask = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * mask_dilates + 1, 2 * mask_dilates + 1))

        # 3. 执行膨胀
        if flow_mask_dilates > 0:
            flow_mask_img = cv2.dilate(mask_binary, kernel_flow)
        else:
            flow_mask_img = mask_binary

        if mask_dilates > 0:
            mask_dilated_img = cv2.dilate(mask_binary, kernel_mask)
        else:
            mask_dilated_img = mask_binary

        # [修改 3] (可选) 边缘羽化/平滑
        # 如果你发现边缘依然硬，可以取消下面这行的注释，给 mask 边缘一点点模糊
        # mask_dilated_img = cv2.GaussianBlur(mask_dilated_img, (3, 3), 0)

        # 归一化回 0-255 并转为 PIL
        flow_masks.append(Image.fromarray(flow_mask_img))
        masks_dilated.append(Image.fromarray(mask_dilated_img))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--mask_dilation', type=int, default=2, help='Mask dilation for video and flow masking.')
    parser.add_argument(
    ...
    # [新增参数] 支持自定义模型路径
    parser.add_argument(
        '--model_path', type=str, default=None, help='Path to the custom model checkpoint.')
    ...
    with torch.no_grad():
        ...
        # [双流准备]
        # masked_frames_for_flow: 挖空，给 RAFT
        masked_frames_for_flow = frames * (1 - masks_dilated)
        # raw_frames_for_model: 原图，给 ProPainter
        raw_frames_for_model = frames
        ...
        if frames.size(1) > short_clip_len:
            ...
            for f in range(0, video_length, short_clip_len):
                ...
                if f == 0:
                    # [修改] 使用挖空图
                    flows_f, flows_b = fix_raft(masked_frames_for_flow[:, f:end_f], iters=args.raft_iter)
                else:
                    # [修改] 使用挖空图
                    flows_f, flows_b = fix_raft(masked_frames_for_flow[:, f - 1:end_f], iters=args.raft_iter)
            ...
            # [修改] 使用挖空图
            gt_flows_bi = fix_raft(masked_frames_for_flow, iters=args.raft_iter)
            torch.cuda.empty_cache()

        if use_half:
            frames, flow_masks, masks_dilated = frames.half(), flow_masks.half(), masks_dilated.half()
            gt_flows_bi = (gt_flows_bi[0].half(), gt_flows_bi[1].half())
            # 如果 masked_frames_for_flow, raw_frames_for_model 也需要转
            raw_frames_for_model = raw_frames_for_model.half()
        ...
        # ---- image propagation ----
        # [修改] 使用原图，不挖空！让模型看到残余像素
        # masked_frames = frames * (1 - masks_dilated)
        masked_frames = raw_frames_for_model
        ...
        if video_length > subvideo_length_img_prop:
            ...
            for f in range(0, video_length, subvideo_length_img_prop):
                ...
                prop_imgs_sub, updated_local_masks_sub = model.img_propagation(masked_frames[:, s_f:e_f],
                                                                               ...
                                                                               'nearest')
                # 更新逻辑：这里需要小心。updated_frames 本质是 Transformer 的输入。
                # Transformer 输入应该是：Known 区域是原图，Unknown 区域是 Prop 填补的图
                # 如果我们用原图 masked_frames，那么 Known 区域已经是原图了。
                # 但 Prop 填补的区域需要替换进去。
                # raw_frames_for_model 已经包含了原图。
                updated_frames_sub = raw_frames_for_model[:, s_f:e_f] * (1 - masks_dilated[:, s_f:e_f]) + \
            ...
            prop_imgs, updated_local_masks = model.img_propagation(masked_frames, pred_flows_bi, masks_dilated,
                                                                   'nearest')
            # 使用原图进行混合
            updated_frames = raw_frames_for_model * (1 - masks_dilated) + prop_imgs.view(b, t, 3, h, w) * masks_dilated
